    ...
    public class CompactAttributePanelDrawable implements Drawable, Element, Selectable {
        ...
        private static final Identifier CHANGED = Identifier.of("kevs-attributes-panel", "textures/gui/changed.png");
        ...
        private void handleStatHover(TextRenderer tr, int innerX, int y, StatRow row, int mouseX, int mouseY) {
            ...
            if (row.inBonusesGroup) {
                ...
                int cleanNameW = (int)Math.ceil(tr.getWidth(split.cleanName) * fs);
                int spaceW = (int)Math.ceil(tr.getWidth(" ") * fs);
                int openParenW = (int)Math.ceil(tr.getWidth("(") * fs);
                ...
                int valuePartW = (int)Math.ceil(tr.getWidth(bonusCountText) * fs);
        ...
        private void showCalculationTooltip(StatEntry stat, int mouseX, int mouseY) {
            ...
            if (! instance.getModifiers().isEmpty()) {
                ...
                for (EntityAttributeModifier mod : instance.getModifiers()) {
                    ...
                    if (lowerPath.startsWith("enchantment.") || lowerPath.startsWith("enchantment/") || lowerPath.contains("enchantment")) {
                        ...
                        if (enchKey != null && ! enchKey.isBlank()) {
                            try {
                                ...
                                if (root.mc().player != null && root.mc().player.getWorld() != null) {
                                    var drm = root.mc().player.getWorld().getRegistryManager();
                                    Registry<Enchantment> enchantmentRegistry = drm.get(RegistryKeys.ENCHANTMENT);

                                    RegistryKey<Enchantment> enchantKey = RegistryKey.of(RegistryKeys.ENCHANTMENT, enchId);
                                    var entryOpt = enchantmentRegistry.getEntry(enchantKey);

                                    if (entryOpt.isPresent()) {
                                        var enchEntry = entryOpt.get();
                                        int lvl = Math.max(1, (int) Math.round(Math.abs(mod.value())));
                                        enchIconStack = EnchantedBookItem.forEnchantment(new EnchantmentLevelEntry(enchEntry, lvl));

                                        Enchantment enchantmentValue = enchEntry.value();
                                        enchDisplayName = enchantmentValue.description().copy().formatted(Formatting.AQUA);
                                        enchantParsed = true;
                                    }
                                }
                    ...
                    if (! foundSource && enchantParsed) {
                        ...
                        lines.add(useName.copy().append(" ").append(Text.literal(opText).formatted(color)));
                        icons.add(useIcon);
                        texIcons.add(null);
                        foundSource = true;
                        continue;
            ...
            if (! hasAnyModifiers) {
                lines.add(Text.literal("None").formatted(Formatting.GRAY));
                icons.add(ItemStack.EMPTY);
                texIcons.add(null);
            }
        ...
        private void buildLayout() {
            ...
            for (var headerDef : cfg.headers) {
                ...
                for (var se : headerLayout.stats) {
                    Identifier icon = headerLayout.iconFor(se);
                    if (icon == null && hasModifiers(se)) {
                        icon = CHANGED;
                    }
        ...
        private void drawStat(DrawContext ctx, TextRenderer tr, int innerX, int y, StatRow statRow) {
            ...
            if (statRow.inBonusesGroup) {
                ...
                int drawY = (int)(y / fs);
                ctx.drawTextWithShadow(tr, bonusText, nDrawX, drawY, colorBody());
                ctx.getMatrices().pop();
            } else {
        ...
        private static class Item {
            ...
            static Item statBonus(StatEntry se, Identifier icon) {
                Item it = new Item(ItemType.STAT, ROW_H_TEXT);
        ...
        private static class StatRow {
            ...
            StatRow(StatEntry se, Identifier icon, boolean bullet, boolean inBonusesGroup) {
                this.stat = se;
                this.icon = icon;
                this.bullet = bullet;
